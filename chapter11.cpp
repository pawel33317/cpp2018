...
class Base1LocalConstructorVariable{ public:
    Base1LocalConstructorVariable(){cout << "Base1LocalConstructorVariable Constructor\n";}
    ~Base1LocalConstructorVariable(){cout << "Base1LocalConstructorVariable Destructor\n";}};
...
class Derived1LocalConstructorVariable{ public:
    Derived1LocalConstructorVariable(){cout << "Derived1LocalConstructorVariable Constructor\n";}
    ~Derived1LocalConstructorVariable(){cout << "Derived1LocalConstructorVariable Destructor\n";}};
...
void konstruktoryIInicjalizacja()
{
    ...
    //3. Znalezienie przez kompilator zdefiniowanego wywołania konstruktora Base
        //wywołanie konstruktora ba se z parametrem a
...
void specyfikatoryDostepu()
{
    ...
    //public -> private
    //private -> inaccessible
    //ale klasa bazowa ma i tak do tego dostęp jedynie jest niewidoczne na zewnątrz
    ...
    //Klasa c dziedzicząc publicznie po b nie ma dostępu do memberów publicznych a
    //jeżeli b dziedziczyło po a prywatnie

    //Klasa i friendy mają zawsze dostęp do mmeberów niedziedziczonych
    //specyfikator dostępu wpływa jedynie na zewnętrzne wywpłania i klasy dziedzoczace

    //klasa dziedzicząca może zmieniać specyfikator dla memberów klasy bazowej
    //sama ma i tak do tego dostęp jedynie zewnętrzne 
    //wywołania i klasy dziedziczące mogą utracić
}
...
void dodanieNowejFunkcjonalnosciIwywolywanieDziedziczonychFunkcjiIoverriding()
{
    ...
    //Obiekt klasy bazowej nie ma dostępu do memberów dodanych w klasie dziedziczacej

    //!!!WYWOLANIE ODZIEDZICZONEJ METODY NA KLASIE DERIVED
    //!!!kompilator sprawdza czy nie ma tekigo membera najpierw w klasie derived
    ...
    //!!!z klasy bazowej a nadają im własny, więc można zrobić z public private
    //i odwrotnie
    cout << "----------------------------\n\n";
    Base2 b2;
    b2.identify();
...
public: //OKRESLA TU SPECYFIKATOR DLA USINGA !!!
    //UGING DEKLARACJA
    using Base3::protectedBaseFunc;//UWIDACZNIA PROTECTED METODE DO PUBLIC !!!
...
private:
    ...
    void test(){
        cout << public_base_variable;    //po using dalej dostępna wewnętrznie
        //base_public_function(); ERROR //po delete nie dostępna wewnętrznie
        Base3::base_public_function();  //ale dostępna przez base
    }
};
void ukrywaniePokazanieOdziedziczonejFunkcjonalnosci()
{
    ...
    //Ukrycie finlcjonalności z base
...
void wielodziedziczenie()
{
    ...
    Derived21020 d21020;
    d21020.getID();//wykonuje specjalnie dodaną funkcję w derived przesłaniającą basey

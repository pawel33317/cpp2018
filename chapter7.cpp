...
{
    //tu możne zmienić adres wskaźnika
    ptrRef = nullptr;
}
void funParamsAndArgs(int i)//pparameter -> zadeklarowany w deklaracji funkcji
{
    ...
    //!!!const za to r-values, l-values i const l-values
    //!!!przez referencję do klas, struktur, jeżeli trzeba modyfikować, jeżeli potrzebny dostęp do danych o tablicy """int (&array)[5]"""
    int* ii = new int[1];
    *ii = 5;
    vunPtr(ii);
    printf("i is still: %d\n", i);
    ...
    //!!!przekazywać przez pointery gdy: przekazujemy wbudowane tablice i pointery które mogą być nullptr
    //!!!
...
void zwracanieWartosci()
{
    ...
    //używać do zmiennych zadeklarowanych wewnątrz funkcji i argumentów otrzymanych przez wartość
    ...
    int* z = getMemory(55);
    delete[] z;
    //używać do zmiennych zaalokowanych dynamicznie zwracających pamięć i argumentów otrzymanych przez ptr

    //zwracanie przez referencję używać do
    //otrzymany parametr jako referencję i klas które nie będą zniszczone
    ...
    //jeżeli zwracanie kilku wartości następuje w kilku miejscach lepiej zrobić strukturę
...
void inlineFunc()
{
    //funlcje zwykłe spowalniają program bo procesor musi do nich skoczeć i odłożyć wartości
    //dla dużych funkcji jest to OK
    ...
    //plik cpp który je includuje, skompuluje a linkerwywali błąd że jest ich wiele
...
void functionOverloading()
{
    ...
    int e = funA(444);//można wykonać bo 444 nie może być nie const referencją bo jest r-value
    ...
    //wywołanie może być ambiguous jeżeli są 2 funkcje które pasują do no np standardowej czy developera konwersji
    //abo ominąć można np scastować argument przy wywołaniu, dodać nową fun lub dodać np 0u czy 5.5f

    //przy wielu parametrach funkcja musi mieć przynajmniej jeden parametr lepszy i resztę nie gorszą
}
...
void defaultParameters()
{
    ...
    //nie można refeklarować domyślnych parametrów
    //czyli może byc albo w definicji funkcji albo w forward deklaracji

    //!!!jeżeli funkcja ma forward deklaracje to tam dodać domyślne parametry a nie w definicji
...
#include <functional>
void funPtrTest(){}
const int funPtrTestI(){printf("Funkcja zwracajaca const int\n"); return 4;}
void callFunc(const int (*const fun)()){fun();}
void callFuncDefaultParam(const int (*const fun)() = funPtrTestI){fun();}
int funPtrD(int* i, double d){printf("Wywolano mnie przez wskaznik d = %f\n", d);return 555;}
void functionPointers()
{
    ...
    //nie constowy wskaźnik na funkcję, która nie przyjmuje parametrów i zwraca voida
    ...
    //!!!jednak niejawnie kompilator nie scastuje wskaźników na funkcje 
    //na wskaźnbiki na voida i odwrotnie
...
void stackAndHeap()
{
    ...
    //CODE SEGMENT (TEXT) -> read only miejsce gdzie skompilowany kod jest w pamięci
    ...
    //HEAP -> dla zmiennych alokowanych dynamicznie
        ...
        //duży obszar pamięci
    //CALL STACK (STACK) -> dla zmiennych lokalnych, parametrów funkcji, i innych informacji związanych z funkcjami
        //szybka alokacja
    ...
    //jak wywołanie funkcji jest znajdowane jest ona wrzucana na stos -> widać całą kolejność wywołania
    ...
    //STACK FRAME -> element wrzucany na stos
        //pilnuje informacji o wszystkich elementach powiązanych z wywołaniem funkcji
    ...
    /*
        ...
        ---> return value jest różnie obsługiwane czasami jako część STACK FRAME czasami jako osobny rejestr
...
void vectorAndStack()
{
    //w tablicach trzeba pamiętać ile elementów używamy bo może być większa zaalokowana pamięć niż potrzebna
...
void recursion()
{
    ...
    //każdy problem rekurencyjny można rozwiązać iteracyjnie ale czasami bardzo trudno
    //iteracyjne prawie zawsze są bardziej wydajne
    //!!!unikać rekurencji chyba że implementacja będzie prostsza
}
...
void errorsCerrExit()
{
    ...
    //jak coś nie tak można zwrócić -1 lbb exit() z cstdlib
...
#define NDEBUG //ignoruje teraz wszystkie aserty
#include <cassert>
void assertStaticAssert()
{
...
void ellipsisVaList()
{
    //!!!funkcja która używa elipsis "..." musi mieć chociaż jeden notmalny parametr
    //... muszą być na końcu
    ...
    //!!!miżemy podać zbyt mało argumentów i odczyta spam ze stosu

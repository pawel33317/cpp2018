...
void klasy()
{
    ...
    //!!!więc funkcja, która moa dostęp do prywatnych pól ma też dostęp do tych pól
    ...
    //enkapsulacja - ukrywanie szczegułów przed użytkownikiem i udostępnianie 
    //tylko publicznego interfejsu

    //ograniczać dostęp do funkcji jeżeli niepotrzebny
    //!!!getter powinny zwracać przez vartość lub const reference a nie przez non-const ref
}
...
void konstruktory()
{
    //!!!jak zmienne są prywatne nie zadziała initializer list ani uniform initialization
    //konstruktor ma nazwe klasy i brak typu zwracanego
    //!!!default condtruktor, bezparametrowy

    C c{4};//!!!uniform initialization odpala konstruktor
    //!!!używać direct initialization dla klas
    ...
    //!!!konstruktor nei tworzy obiektu tylko kompilator
...
public:
    ...
    int g1 = 44;//od C++11 
    int g {44};//od C++11 
    //!!!jeżeli będzie na liście inicjalizacyjnej g() to konstruktor pominie wartość domyślną
    //!!!używać wartości domyślnych, konstruktor ich użyje

    QQ(int i) : a{i}, arr{1,2}, p(3.14) {};//przed c++11 działało tylko {} do zerowania
    //są inicjalizowane w kolejności deklaracji w klasie najpierw a -> arr -> p
    //!!!dobrze inicjalizować w tej samej kolejności
};
void listaInicjalizacyjnaKonstruktora()
{
    QQ q{55};
...
public:
    ...
    //WW(bool) : WW(), z(4) {} //nie można zainicjalizowac pola bo delegujemy konstruktor
...
{
    WW ww(0);
    //!!!przed c++11 nie wilno bylo wywolywac konstruktora w konstruktorze
...
void destruktory()
{
    ...
    //!!! RAII -> technika programowania w której użycie zasobu jest powiązane z czasem
    //życia obiektu z automatycznym czasem trwania (niedynamicznymi)
...
public:
    ...
    //!!!nie używaż this w klasach chyba że konieczne
...
void thisHiddenPtr()
{
    ...
    //kompilator dodaje niejawnie do każdej metody parametr -> constowy wskaźnik "this"
    //który trzyma adres do obiektu metody na którym została wykonana
    //konpilator zamienia odniesienia do pól w metodach mId = 3 ==> this->mId = 5
...
void klasywCppiHpp()
{
    ...
    //inline funkckcje są wyłączone z one definition rule bo ot nie funkcje

    //w programie obiekty lub non-inlline functions nie mogą mieć więcej niż jednej definicji

    //definicja klasy w headerze o nazwie klasy i dodatkowo header guardy 
    //definicje w cpp o nazwie klasy

    //umieszczenie dużego kodu metody w cpp powoduje:
        //duży rozmiar hpp
        //bądą inline ...
        //zmiana czegokolwiek w hpp będzie wymagała przekompilowania innych które ją includują
    ...
    //zwalidował czy program jest składniowio poprawny, implementacja jest
    //w prekompilowanym pliku i dołączana do kompilacji, nie zobaczymy kodu
...
public:
    ...
    int& getZ() { return ++z; }
    //void setZ(int zz) const { z = zz; } nie skompiluje sie bo modyfikuje member var
    void setZ(int zz) { z = zz; }
    int setZ(int zz, double b) { z = zz; return 0; } //ovewrloadowane fun mogą mieć inne typy
};
class GG
{
    public:
    const int a;
    //const int b; -> jeżeli b byłoby const byłaby wymagana inicjalizacja w konstruktorze
    int b;
    GG(int abc) : a(44) {}
};
void constKlasy()
{
    //!!!const member funkcja gwarantuje że nie modyfikuje obiektu i nie wywołuje
    //non-const metod

    //jeżeli funkcja ma definicje w pliku cpp const ma być dodane do obu hpp i cpp

    //konstruktor nie może być const bo inicjalizuje pola
    //const obiekt nei musi inicjalizować wszystkich pól jak zwykły
    const RR rr(123);
    GG gg {8};
    ...
    //częściej niż tworzenie const obiektór przekazuje się zwykłe jako const referencję
...
{
public:
    static int staticVar;//zmienna współdzielona przez wszystkie obiekty
    static const int constStatic = 12345;//CONSTA STATICA można zdefiniować w klasie
...
int SS::staticVar = 1234;//obowiązkowe -> definicja zmiennej statycznej klasy
//int SS::staticVar;//inicjalizacja zerem
int SS::privateStaticVar;//prywatną też trzeba zdefiniować i zainicjalizować
void staticMember()
{
...
class KlasaZeZmiennaStaticDoZainicjalizowania
{
    ...
    private:
        ...
        static _inicjalizator init;//tylko poto aby wykonać konstruktor klasy nested
};
KlasaZeZmiennaStaticDoZainicjalizowania::_inicjalizator KlasaZeZmiennaStaticDoZainicjalizowania::init;
//wywołuje konstruktor klasy nested _inicjalizator a ten inicjalizuje też inne pola
...
void staticMethod()
{
    printf("Zeby odczytac wartosc musialem stworzyc obiekt %d\n", DD().getDDStaticVar());
    ...
    //!!!funkcje z tylko statycznymi memberami są nazywane monostates,
...
class RRR;//class prototype aby PPP wiedziala ze jest takie cos jak RRR
...
//trzeba podzielić definicje od deklaracji i deklarację klasy dać wyżej
class PPP
{
public:
...
private:
    int privateVar;
    friend void displayPppContent(const PPP& ppp);//oznacz displayPPPContent jako friend
...
void friendKlasy()
{
    //!!!friend klasa/funkcja to funkcja/klasa która ma dosteę do prywatnych 
    ...
    //!!!limotować friend funkcje i klasy bo naruszają enkapsulecję
    printf("======================\n");
}
...
void obiektyAnonimowe()
{
    //obiekty anonimowe to wartości bez nazwy, nie ma możliwości aby się do nich odnieść
    //mają expression zakres, są tworzone, wykonywane i niszczone w pojedynczym wyrażeniu
...
void obiektyAnonimowe2()
{
    ...
    printf("Przedluzony czas życia obiektu anonimowego poprzez przypisanie const ref\n");
    printf("Function end\n");
}
...
void typyZagniezdzone()
{
    ...
    //klasy zagnieżdżone mają taki sam dostęp do elementów klasy otaczającej jakie obejmuje klasa otaczająca
    //!!!klasa zagnieżdżona nie ma thisa do klasy otaczającej

    //żadko stosowane np do iteratorów
}
...
private:
    ...
    //precyzyjny zegar ale może być to alias na std::chrono::system_clock lub steady_clock

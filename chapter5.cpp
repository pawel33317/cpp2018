...
{
   //#include <cstdlib>
   //exit(0);..konczy program i zwraca 0 do systemu operacyjnego
}
...
void switche()
{
   ...
   switch(Kolor::BLACK)
   {
      case Kolor::RED://Kolor::RED jest zdefiniowane jako const int i testowane ==
         {
            int g = 5;
            nicNieRob(&g);
         }
   ...
   //!!!w switchu nie można inicjalizować zmiennej bo initjalizacja wymaga wykonania a może nie zostać wykonana !!!
   ...
   //!!!ale są widoczne dopiero od momentu zadeklarowania, więc deklaracja zmiennej mówi jedynie kompilatorowi że od tego momentu może jej używać
...
void gotoStatement()
{
   ...
   z = 5;
   nicNieRob(&z);
   //int x = 55; !!!jeżeli zmienna będzie inicjalizowana nie można skoczyć za nią
skip:
...
void petle()
{
   ...
   while (--i > 0)
   {
      //infinite loop
      break;
      //każda zmienna tworzona w pęcli jest w bloku więc jest tworzona i usuwana co iterację
   }
   ...
   do
   {
      printf("Wewnarz do while\n");
      bool b = false;
      nicNieRob(&b);
   }
   while(true == false);//nie można użyć b bo została zniszczona
   ...
   //for(i = 0, j = 10; j > i; --j, ++i)//"," operator jest głównie i jedynie wykorzystywany w forze
...
#include <cstdlib>
#include <random>
int getRandomNumber(int min, int max)//chujowa metoda bo mniejsze liczby są bardziej prawdopodobne
...
void randomNumbers()
{
   ...
   srand(0);//wywoływana raz ustawia seed(jakąś zmienną statyczną counter wewnątrz siebie) value na jakąś wartość ()
   int r = rand();//losuje wartość od 0 do RAND_MAX ZAWSZE BEDZIA TAKA SAMA bo jak wyżej
   //każda wylosowana liczba jest zależna od poprzedniej 
   printf("Wylosowana liczba to: %d\n", r);

   srand(static_cast<unsigned int>(time(0)));//teraz przy każdym uruchomieniu liczby będą inne bo wartość startowa będzie inna
   r = rand();
   printf("Wylosowana liczba to: %d\n", r);
   printf("Liczba losowa z przedzuału 5-10 to: %d\n", getRandomNumber(5,10));

   //w c++11 jest dodane bardzo wiele generatorów liczb losowych różnych algorytmów itp

   std::random_device rd; //tym będzie inicjalizowana pierwsza liczba -> w przypadku sranda był to np czas
   std::mt19937 mersenne(rd());//jeden z algorytmów podobno dobry
   std::uniform_int_distribution<> generator(1,10);//Generator
   std::uniform_real_distribution<double> generator2(0.3, 0.99); 
   printf("Liczba wygenerowana z użyciem random_device jako seeda, algorytmem mersenne i generatorem intow %d\n", 
      generator(mersenne));
   printf("Wygenerowana liczba zmiennoprzecinkowa <0.3, 0.99>=%f\n", generator2(mersenne));
}

void wprowadzanieDanych()
{
   //!!!robust - program obsługujący błędy (przewidujący nadużycia i obsługujący je)
   //dane odczytywane przez operator extrakcji << są tymczasowo przechowywane w buforze zanim trafią do zmiennej

   //użycie operatora >> powoduje przypisanie danych z bofora do zmiennej
   ...
   //std::cin.ignore(32767, '\n'); wywala z bufora wszystko poza wczytaną daną do zmiennej
